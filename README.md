...
**Why** Hobgoblin?  
- many existing engines are built for general purpose game creation. engines such as unity, unreal, and godot do not cater to memory determinism. for large scale simulations with many long lived AI agents, these engines are not sustainable. this includes network level determinism. as such, hobgoblin aims to provide a high degree of optimization and scalability in regard to memory for deterministic simulations that other engines do not provide.
## REQUIREMENTS
### BUILD ENVIRONMENT
